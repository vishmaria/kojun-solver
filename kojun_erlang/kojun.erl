...
restricaoPossibilidades(Tab, Reg, {X,Y}) ->
    ...
    PossibilidadesBasicas = lists:seq(1, tamanhoRegiao(Reg, {X,Y})), %Lista de possibilidades inicia como uma lista com todos os numeros de 1 até (tamanho da região)
    ...
    PossibilidadesFiltrada2 = lists:subtract(PossibilidadesFiltrada1, celulasAdjacentes(Tab,{X,Y})), %São removidos da lista valores presentes em células adjacentes
...
%Chama valoresUnicos e realiza a atualização da matriz de valores caso existam valoresUnicos.encontraUnicaPossibilidade
...
testaPossibilidades(Tab,Reg,{X,Y}, [Possibilidade | Resto])->
    Novo_tab = atualizaTabuleiro(Tab,{X,Y},Possibilidade), %cria uma matriz de valores com uma possibilidade 'chutada'
...
valoresPossiveisDaRegiao(Tab,Reg,Reg_id)->
    ...
    Coords_zeros = lists:filter(fun({X,Y})-> valorCoordenada(Tab, {X,Y}) == 0 end, Coordenada),
...
preSolucionador(Tab, Reg, [{X,Y} | Resto])->
    Novo_tab1 = encontrarUnicaPossibilidade(Tab,Reg), %Cria um novo tabuleiro com valores unicos (caso não existam nenhum, irá retornar o mesmo tabuleiro que o informado)
